...
const LEFT_EDGE_THRESHOLD = 0.15; // 15% of screen width
...
export const StoryViewer: React.FC<StoryViewerProps> = ({
  ...
  }, [storyIndex, groupIndex, currentGroup, storyGroups, isTransitioning, isNavigating, performTransition]);
  ...
  const handleTouchEnd = useCallback((direction: 'next' | 'prev' | null) => {
    if (isTransitioning || isNavigating) return;
  ...
  const handleTap = useCallback((x: number) => {
    ...
    if (tapPosition <= LEFT_EDGE_THRESHOLD) {
      goToPreviousStory();
    } else {
      goToNextStory();
    }
  }, [goToNextStory, goToPreviousStory, isTransitioning, isNavigating]);
